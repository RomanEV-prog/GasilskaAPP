...
- **Varnost (od 30. 7. 2026):** 2FA (TOTP) za web portal — vklop v Nastavitvah, prijava vrne `requires2fa`+`pendingToken` → `/auth/2fa/verify`. **Mobilna app 2FA še ne podpira** — član z vklopljeno 2FA se vanjo ne more prijaviti. TOTP je lastna implementacija (`auth/totp.util.ts`) — `otplib` v13 je ESM-only in se lomi v Jest/node:20, NE vračaj ga. `token_version` v refresh žetonu: sprememba gesla/2FA odjavi vse naprave. Helmet na backendu, varnostni headerji+CSP v `frontend/Caddyfile`, Swagger na produkciji izklopljen. Glej `docs/DECISIONS.md` ADR-011.
